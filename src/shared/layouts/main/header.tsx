...
export const Header = () => {
    ...
    const NavMobile: React.FC<NavMobileProps> = ({ icon, label, link }) => (
        <div className="group relative px-4 cursor-pointer">
            <Link
                href={link}
                className={`flex h-10 w-10 items-center justify-center rounded-full hover:text-c-green opacity-70 ${pathname.startsWith(link) ? 'text-primary' : ''
                    }`}
            >
                {icon}
            </Link>
    ...
    return (
        <header
            className={`fixed top-0 left-0 right-0 z-50 flex justify-between h-20 items-center px-8 transition-all duration-300 ${isScrolled ? 'bg-white shadow-lg h-16' : 'bg-transparent'
                }`}
        >
